...
```{r}
train <- read.csv("/Users/yeoyoung/Documents/GitHub/Kaggle/House Price/all/train.csv", stringsAsFactors = F)
test <- read.csv("/Users/yeoyoung/Documents/GitHub/Kaggle/House Price/all/test.csv", stringsAsFactors = F)
```
...
```{r}
#내 생각
cor(all$Fence, all$SalePrice, use='complete.obs')
```
**Please return to the 5.2 Tabs menu to select other (groups of) variables**
...
```{r}
#내분석
all[,c("LotFrontage","LotArea","LotShape","Neighborhood")] %>% filter(is.na(all$LotFrontage))  
#summarise(group_by(all[!is.na(all$LotFrontage),], all[!is.na(all$LotFrontage),all$Neighborhood]), mean=mean(all))
...
```
```{r}
boxplot(all$LotFrontage, all$Neighborhood)
```
...
```{r}
#내 생각
ggplot(all[!is.na(all$SalePrice),], aes(x=as.factor(LotShape), y=SalePrice)) +
        geom_bar(stat='summary', fun.y = "median", fill='blue')+
        scale_y_continuous(breaks= seq(0, 800000, by=100000), labels = comma) +
        geom_label(stat = "count", aes(label = ..count.., y = ..count..))
...
```{r}
#내 생각
cor(all$SalePrice, all$LotShape, use='complete.obs')
```
